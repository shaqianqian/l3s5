...
annotate_types_term(_Connection,'$NULL'(_ID),_T,Ts,Ts) :- 
  !.
% Dates:
annotate_types_term(_Connection,date(Y,M,D),datetime(date),Ts,Ts) :- 
  !,
  integer(Y),
  integer(M),
  integer(D).
% Times:
annotate_types_term(_Connection,time(H,Mi,S),datetime(time),Ts,Ts) :- 
  !,
  integer(H),
  integer(Mi),
  integer(S).
% Times:
annotate_types_term(_Connection,datetime(Y,M,D,H,Mi,S),datetime(datetime),Ts,Ts) :- 
  !,
  integer(Y),
  integer(M),
  integer(D),
...
is_type(datetime(date)).
is_type(datetime(time)).
is_type(datetime(datetime)).
...
get_internal_type(UserType,InternalType) :- % :::WARNING: Uncommented for open types (null type) as in DES-RA> impar(x):= select true (impar); 
  var(UserType),
  var(InternalType),
  !,
...
get_expr_type(date(_Y,_M,_D),datetime(date)) :- !.
get_expr_type(time(_H,_M,_S),datetime(time)) :- !.
get_expr_type(datetime(_Y,_M,_D,_H,_Mi,_S),datetime(datetime)) :- !.
...
get_expr_type(C,T) :- 
  C =.. [F|As],
  length(As,Arity),
  function(F,_,_,arithmetic,[T|_],Arity),
  !.
...
% get_expr_type(C,T) :- 
%   C =.. [F,L,R],
%   my_infix_operator(F,_SOP,_PO,[T,LT,RT],_D,_P,_A),
%   my_ground(T),
%   !.
% An infix with a type depending on its arguments
get_expr_type(C,T) :- 
  C =.. [F,L,R],
  my_infix_operator(F,_SOP,_PO,[T,TL,TR],_D,_P,_A),
%  !, % Operator overloading makes this to move to the end
  get_expr_type(L,TL),
  get_expr_type(R,TR),
...
get_expr_type(A,A) :- % A type already substituted
  (is_type(A)
   ->
    true
   ;
    my_raise_exception(A,'Incompatible types:',[])).
  
combine_types(TL,TR,T) :-
  combine_number_types(TL,TR,T).
combine_types(TL,TR,T) :-
  combine_string_types(TL,TR,T).
combine_types(TL,TR,T) :-
  combine_datetime_types(TL,TR,T).
  
% Combine two string types
combine_string_types(string(_),string(_),string(_)).

% Combine two datetime types
combine_datetime_types(datetime(_),datetime(_),datetime(_)).
combine_datetime_types(number(_),datetime(DT),datetime(DT)).
combine_datetime_types(datetime(DT),number(_),datetime(DT)).


% Combine two number types
combine_number_types(TL,TR,number(integer)) :-
  type_casting(off),
  TL=number(integer),
  TR=number(integer),
  !.
combine_number_types(_TL,_TR,number(float)) :-
  type_casting(off),
  !.
combine_number_types(TL,TR,number(Integer)) :-
  type_casting(on),
...
%  (var(Integer) -> des_atts:put_atts(Integer,+integer) ; Integer==integer),
  ...
  !.
combine_number_types(TL,TR,number(IntOrFloat)) :-
  type_casting(on),
