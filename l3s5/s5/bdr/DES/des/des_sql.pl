...
my_cond_factor(not_in(L,R)) --> 
  ...
  my_DQL_or_constant_tuples(A,R),
  my_closing_parentheses_star(N).
my_cond_factor('$like'(L,R)) --> 
  my_sql_expression(L,_),
  my_sql_blanks,
  my_kw("LIKE"),
  my_sql_blanks,
  my_sql_expression(R,_).
my_cond_factor('$like'(L,R,E)) --> 
  my_sql_expression(L,_),
  my_sql_blanks,
  my_kw("LIKE"),
  my_sql_blanks,
...
my_sql_expression(PP,Lo,To) -->
  ...
  !, % WARNING
  my_r_sql_expression(PP,0,L/Lo,T/To).
% my_sql_expression(_PP,_To) -->
%   "--", % Two dashes are intended for single-line comments, neither two applications of the minus prefix nor one to a negative number
%   {!,
%    fail}.
my_sql_expression(PP,Lo,To) -->
...
my_sql_factor(Function,number(_)) -->
  ...
  pop_syntax_error(Old1),
  push_syntax_error(['Expected FROM'],Old2),
  my_sql_blanks,
  my_kw("FROM"),
  pop_syntax_error(Old2),
  my_sql_blanks,
...
my_sql_factor(C,_) -->
  my_column(C),
  {C=attr(_,Name,_), \+ arithmetic_constant(Name)}.
my_sql_factor(F,T) --> 
...
solve_des_sql_query(Lang,create_or_replace_table_as(SQLst,Schema)) :-
...
%  solve_des_sql_query(Lang,create_table(Schema,[])),
  ...
  (relation_exists(Name)
   ...
   ;
    ...
    solve_des_sql_query(Lang,insert_into(Name,Colnames,SQLst)),
    drop_view(TempName)
  ).
...
translate_cond(Scope,C,N,LN,IMap,IMap,IRen,IRen,_RelSGs,DLsts,Goals) :-
  ...
  !.
translate_cond(Scope,C,N,LN,IMap,OMap,IRen,ORen,_RelSGs,DLsts,Goals) :- % For LIKE's
  C=..[Prim|Args],
  translate_expr_varcte_list(Scope,Args,N,LN,IMap,OMap,IRen,ORen,DLsts,VarCtes,Goals1),
  MC=..[Prim|VarCtes],
...
translate_expr_varcte_list(Scope,[Exp],N,LN,IMap,OMap,IRen,ORen,DLsts,[VarCte],Goals) :-
  translate_expr_varcte(Scope,Exp,N,LN,IMap,OMap,IRen,ORen,DLsts,VarCte,Goals).
translate_expr_varcte_list(Scope,[Exp,Exp1|Exps],N,LN,IMap,OMap,IRen,ORen,DLsts,[VarCte|VarCtes],Goals) :-
  translate_expr_varcte_list(Scope,[Exp],N,N1,IMap,IMap1,IRen,IRen1,DLsts1,[VarCte],Goals1),
  translate_expr_varcte_list(Scope,[Exp1|Exps],N1,LN,IMap1,OMap,IRen1,ORen,DLsts2,VarCtes,Goals2),
...
translate_expr(Scope,SQLE,IsAggr,N,LN,IMap,OMap,IRen,ORen,DLsts,Result,Goals) :- 
  SQLE =.. [F|SQLArgs],
  is_non_Prolog_function(SQLE),
  !, 
  translate_expr_list(Scope,SQLArgs,IsAggr,N,LN,IMap,OMap,IRen,ORen,DLsts,DLArgs,Goals1),
...
write_expr(E,I,DBMS) :- 
  E =.. [Op,Arg1,Arg2],
  my_infix_operator(Op,_,POp,_,_,_,_),
  POp\==low,
  !,
