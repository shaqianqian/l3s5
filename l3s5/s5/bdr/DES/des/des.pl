...
display_banner :-
  write_log('*********************************************************'), nl_log,
  write_log('*                                                       *'), nl_log,
  write_log('*        DES: Datalog Educational System v.4.2          *'), nl_log,
  write_log('*                                                       *'), nl_log,
...
%  ((Simplified00==true ; Simplified0==true ; Simplified1==true ; Simplified2==true ; Simplified3==true) -> SCauses=[simplified|TCauses] ; SCauses=TCauses),
  ...
  ((Compiled00==true ; Compiled0==true ; Compiled1==true) -> CCauses=[compiled|SfCauses] ; CCauses=SfCauses),
...
translate_expression_goal(A,TA,Compiled) :-
  ...
  C=..[Op,TL,TR],
  append_goals(LG,RG,TG),
  append_goals(TG,C,TA),
  (TG == true -> true ; Compiled=true).
translate_expression_goal(A,A,_Compiled).
  
translate_expression(E,E,true) :-
  (var(E) ; number(E)), % Some atoms as current_date must be translated
  !.
translate_expression(E,TE,Goals) :-  
  is_non_Prolog_function(E),
  !, 
  E =.. [F|EArgs],
  translate_expression_args(EArgs,DLArgs,Goals1),
...
translate_expression(E,TE,Goals) :-
  E =.. [F|As],
  !, 
  translate_expression_args(As,RAs,Goals),
  TE =.. [F|RAs].
      

translate_expression_args([],[],true).
translate_expression_args([T|Ts],[RT|RTs],Goals) :-
  !, 
  translate_expression(T,RT,Goals1), 
  translate_expression_args(Ts,RTs,Goals2),
...
compute_datalog_query(Query,NVs,CId,Origin) :-
  ...
  !,
  preprocess((':-'(Head,Query),NVs),SiRNVsList,_SfRNVsList,_ExRNVsList,[],_IArgsList,_Modes,exec,datalog,query,Causes,[],Unsafe),
...
my_date(date(Y,M,D)) -->
  ...
  my_blanks_star,
  my_positive_integer_sequence([UY,UM,UD]),
  my_blanks_star,
  ")",
  pop_syntax_error(Old),
  {normalized_date(date(UY,UM,UD),date(Y,M,D))}.

my_time(time(H,M,S)) -->
  "time(",
  push_syntax_error(['Invalid time. Expected integers for hour, minute and second'],Old),
  my_blanks_star,
  my_positive_integer_sequence([UH,UM,US]),
  my_blanks_star,
  ")",
  pop_syntax_error(Old),
  {normalized_time(time(UH,UM,US),time(H,M,S))}.
...
my_datetime(datetime(Y,M,D,H,Mi,S)) -->
  ...
  my_blanks_star,
  my_positive_integer_sequence([UY,UM,UD,UH,UMi,US]),
  my_blanks_star,
  ")",
  pop_syntax_error(Old),
  {normalized_datetime(datetime(UY,UM,UD,UH,UMi,US),datetime(Y,M,D,H,Mi,S))}.
...
my_positive_integer_sequence([I|Is]) -->
  my_positive_integer(I),
  push_syntax_error(['Expected comma-separated sequence of integers'],Old),
  my_blanks_star,
  ",",
  my_blanks_star,
  my_positive_integer_sequence(Is),
  pop_syntax_error(Old).
my_positive_integer_sequence([I]) -->
  my_positive_integer(I).
...
my_factor(FAs,T,Vi,Vo) --> 
  ...
  "(",
  my_blanks_star,
  my_function_arguments(A,As,Ts,Vi,Vo),
  my_blanks_star,
  my_right_parenthesis,
...
my_function_arguments(1,[E],[T],Vi,Vo) -->
  my_expression(E,T,Vi,Vo).
my_function_arguments(1,[T],[type(T)],Vi,Vi) -->
  my_DLtype(T),
  !.
my_function_arguments(A,[E|Es],[T|Ts],Vi,Vo) -->
  {A>1, Es\==[]},
  my_function_arguments(1,[E],[T],Vi,Vo1),
  my_blanks_star,
  ",",
  my_blanks_star,
  {A1 is A-1},
  my_function_arguments(A1,Es,Ts,Vo1,Vo).
...
% function(Name, PrologPredefined, Description, Kind(arithmetic, arithmetic_cte, string, datetime or aggregate), Types(Return type and argument types), Arity)
...
function(round,round,'Closest integer',arithmetic,[number(integer),number(_)],1).
...
function(substr,substr,'Substring: String, offset, and length',string,[string(_),number(_),number(_)],2).
...
function(datetime_sub,datetime_sub,'Return the number of days/seconds between both dates/times',datetime,[number(integer),datetime(date),datetime(date)],2).
...
my_infix_operator('||',"||",'concat',[string(_),string(_),string(_)],'String concatenation',500,yfx).
my_infix_operator('+',"+",'datetime_add',[datetime(_),datetime(_),number(integer)],'Date/time addition between datetimes',500,yfx).
my_infix_operator('+',"+",'datetime_add',[datetime(_),number(integer),datetime(_)],'Date/time addition between number and datetime',500,yfx).
...
%  (N<1 -> my_raise_exception(generic,syntax(['First argument of top must be greater than 0: ',top(N,G)]),[]) ; true),
  (N<1 -> my_raise_exception(top(N,G),'First argument of top must be greater than 0:',[]) ; true),
  dlrule_id(R,RId),
...
compute_primitive(Arithmetic,R) :- 
  compute_arithmetic_primitive(Arithmetic,R),
  !.
compute_primitive(Comparison,R) :- 
  compute_comparison_primitive(Comparison,R),
  !.
compute_primitive(String,R) :- 
  compute_string_primitive(String,R),
  !.
compute_primitive(Datetime,R) :- 
  compute_datetime_primitive(Datetime,R),
  !.
compute_primitive(Value,R) :- 
  compute_conversion_primitive(Value,R),
  !.
  

% Conversion primitive computation: 'cast'
compute_conversion_primitive('$cast'(date(Y,M,D),string(_),C),_R) :-
  !,
  pad_zeroes(Y,4,PY),
  pad_zeroes(M,2,PM),
  pad_zeroes(D,2,PD),
  atomic_concat_list([PY,'-',PM,'-',PD],C).
compute_conversion_primitive('$cast'(time(H,Mi,S),string(_),C),_R) :-
  !,
  pad_zeroes(H,2,PH),
  pad_zeroes(Mi,2,PMi),
  pad_zeroes(S,2,PS),
  atomic_concat_list([PH,':',PMi,':',PS],C).
compute_conversion_primitive('$cast'(datetime(Y,M,D,H,Mi,S),string(_),C),R) :-
  !,
  compute_conversion_primitive('$cast'(date(Y,M,D),string(_),C1),R),
  compute_conversion_primitive('$cast'(time(H,Mi,S),string(_),C2),R),
  atomic_concat_list([C1,' ',C2],C).
compute_conversion_primitive('$cast'(CteDate,datetime(date),Date),_R) :-
  atom(CteDate),
  !,
  atom_to_date(CteDate,Date).
compute_conversion_primitive('$cast'(CteTime,datetime(time),Time),_R) :-
  atom(CteTime),
  !,
  atom_to_time(CteTime,Time).
compute_conversion_primitive('$cast'(CteDatetime,datetime(datetime),Datetime),_R) :-
  atom(CteDatetime),
  !,
  atom_to_datetime(CteDatetime,Datetime).
compute_conversion_primitive('$cast'(date(Y,M,D),datetime(datetime),datetime(Y,M,D,0,0,0)),_R) :-
  !.
compute_conversion_primitive('$cast'(datetime(Y,M,D,_,_,_),datetime(date),date(Y,M,D)),_R) :-
  !.
compute_conversion_primitive('$cast'(datetime(_,_,_,H,Mi,S),datetime(time),time(H,Mi,S)),_R) :-
  !.
compute_conversion_primitive('$cast'(DT,datetime(Type),DT),_R) :-
  DT=..[Type|_],
  !.
compute_conversion_primitive('$cast'(String,number(integer),Integer),_R) :-
  atom(String),
  !,
  atom_codes(String,Codes),
  number_codes(Number,Codes),
  Integer is truncate(Number).
compute_conversion_primitive('$cast'(String,number(float),Float),_R) :-
  atom(String),
  !,
  atom_codes(String,Codes),
  number_codes(Number,Codes),
  Float is float(Number).
compute_conversion_primitive('$cast'(Number,string(S),String),_R) :-
  number(Number),
  !,
  number_codes(Number,Codes),
...
compute_conversion_primitive('$cast'(FromString,string(S),ToString),_R) :-
  atom(FromString),
  !,
  string_type_size(string(S),L),
  (var(L) -> ToString=FromString ; substr(FromString,1,L,ToString)).
compute_conversion_primitive('$cast'(Number,number(float),Float),_R) :-
  number(Number),
  !,
  Float is float(Number).
compute_conversion_primitive('$cast'(Number,number(integer),Integer),_R) :-
  number(Number),
  !,
  Integer is truncate(Number).
compute_conversion_primitive('$cast'(Value,Type,_),_R) :-
  my_raise_exception(cast(Value,Type),'Unsupported type conversion:',[]).
  
% Arithmetic primitive computation: 'is'
compute_arithmetic_primitive(A is B,_R) :-
...
compute_arithmetic_primitive(A is B,R) :- 
%  \+ (contain_null(B)),
  (my_ground(B)
   ->
    A is B
...
compute_string_primitive('$lower'(S,L),_R) :- 
  to_lowercase(S,L).
compute_string_primitive('$substr'(S,O,L,V),R) :- 
  (O<1 -> my_raise_exception(R,'Substring offset must be greater than 0 in ',[]) ; true),
  substr(S,O,L,V).
...
compute_datetime_primitive('$current_date'(date(Y,M,D)),_R) :-
  my_datetime((Y,M,D,_H,_Mi,_S)).
compute_datetime_primitive('$current_time'(time(H,Mi,S)),_R) :-
  my_datetime((_Y,_M,_D,H,Mi,S)).
...
datetime_add(date(Y,M,D),I,date(Y1,M1,D1)) :-
  DI is D+I,
  normalized_date(date(Y,M,DI),date(Y1,M1,D1)).
datetime_add(time(H,Mi,S),I,time(H1,Mi1,S1)) :-
  SI is S+I,
  normalized_time(time(H,Mi,SI),time(H1,Mi1,S1)).
datetime_add(datetime(Y,M,D,H,Mi,S),I,datetime(Y1,M1,D1,H1,Mi1,S1)) :-
  SI is S+I,
  normalized_datetime(datetime(Y,M,D,H,Mi,SI),datetime(Y1,M1,D1,H1,Mi1,S1)).
...
%   it can contain overloaded operators/funcions (as '+' which operates on several data types).
%   Then, it is recompiled and solved
assign(V,E,R) :-
  catch(compute_primitive(V is E,R),M,process_assign_exception(M,V,E,R)). % Let Prolog devise whether E is not an arithmetic expression
...
process_assign_exception(M,V,E,_R) :-
  ...
  CE\=E, % If tried already without success, raise exception
  !,
  solve(TA,1,(-1,[])).
