...
atom_to_date(A,D) :-
  ...
  !.
atom_to_date(A,_D) :-
  my_raise_exception(generic,syntax(['Invalid date: ',A,'. Valid format is YYYY-MM-DD']),[]).
...
my_normalized_date(date(UY,UM,UD),date(Y,M,D)) :-
  my_normalize_datetime(date(UY,UM,UD,0,0,0),date(Y,M,D,0,0,0)),
  !.
my_normalized_date(D,_) :-
...
my_normalized_time(time(UH,UM,US),time(H,M,S)) :-
  my_normalize_datetime(date(2000,1,1,UH,UM,US),date(_,_,_,H,M,S)),
  !.
...
my_normalized_datetime(datetime(UY,UM,UD,UH,UMi,US),datetime(Y,M,D,H,Mi,S)) :-
  my_normalize_datetime(date(UY,UM,UD,UH,UMi,US),date(Y,M,D,H,Mi,S)),
  !.
my_normalized_datetime(D,_) :-
