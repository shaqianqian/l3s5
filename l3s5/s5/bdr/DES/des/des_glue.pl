...
my_normalize_datetime(date(UY,UM,UD,UH,UMn,US),date(Y,M,D,H,Mn,S)) :-
  date_time_stamp(date(UY,UM,UD,UH,UMn,US,0,TZ,DST),TS),
  stamp_date_time(TS,date(Y,M,D,H,Mn,FS,_O,TZ,DST),'UTC'),
  S is integer(FS).
...
my_ordered_datetime_sub_seconds((Y1,M1,D1,H1,Mn1,S1),(Y2,M2,D2,H2,Mn2,S2),Seconds) :-
  date_time_stamp(date(Y1,M1,D1,H1,Mn1,S1,0,-,-), When1),
  date_time_stamp(date(Y2,M2,D2,H2,Mn2,S2,0,-,-), When2),
  Seconds is truncate(When1-When2).  
