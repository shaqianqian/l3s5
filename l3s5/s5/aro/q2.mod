...
subject to limit_hanger1:
hanger_used*20>=qte_vache;
subject to limit_hanger2:
(hanger_used-1)*20<=qte_vache;
maximize profit :
(sum {v in VEG} ((are_veg[v]*rendement[v]-consommation[v]*qte_vache) * prix_vente[v]))+(qte_vache*200)+((5-hanger_used)*1500);
data;
