...
int parseline(char *cmdline, char *token[MAXCMDS][MAXARGS], int *nbcmd) {
    ...
    /* We first check whether cmdline contains a no-break space, a
     * common mistake, hard to spot */
    nbsp = strstr(cmdline, " ");
    if(nbsp != NULL) {
        fprintf(stderr, BOLD "Warning" NORM ": your command line contains a no-break space\n");
        fprintf(stderr, "  %s\n", cmdline);
        fprintf(stderr, "  %*c\n", (int)(nbsp-cmdline+1), '^');
        fprintf(stderr, "It is probably not what you wanted to run.\n");
    }
