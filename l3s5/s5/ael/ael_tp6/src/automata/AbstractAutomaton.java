...
public abstract class AbstractAutomaton implements Recognizer, AutomatonBuilder {
	...
	@Override
	abstract public void setInitial(State s) throws StateException ;

	@Override
	public void setInitial(String name) throws StateException {
		setInitial(states.get(name));
	}

	@Override
	public void setInitial(int id) throws StateException {
		setInitial(states.get(id));
	}

	@Override
	public abstract boolean isInitial(State s) throws StateException ;

	@Override
	public abstract void addTransition(State from, Character letter, State to) throws StateException ;

	@Override
	public void addTransition(String from, Character letter, String to) throws StateException {
		addTransition(states.get(from), letter, states.get(to));
	}

	@Override
	public void addTransition(int from, Character letter, int to) throws StateException {
		addTransition(states.get(from), letter, states.get(to));
	}
	...
	protected StringWriter imageWriter() {
		...
		for (State from : getStates()){
			for (char letter : usedAlphabet()){
				for (State dest : getTransitionSet(from,letter)){
					out.println(from +","+letter+"->"+dest);
				}
			}
		}
		return buff;
	...
	public Writer writeGraphvizTransitions(Writer buff) {
		...
		for (State from : getStates()){
			for (char letter : usedAlphabet()){
				for (State dest : getTransitionSet(from,letter)){
					out.print("  " + from.getId() + " -> " + dest.getId());
					out.println(" [label = \"" + letter + "\" ]");
				}
			}
		}
		return buff;
