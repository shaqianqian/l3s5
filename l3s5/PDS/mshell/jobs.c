/* ------------------------------
   $Id: jobs.c,v 1.2 2005/03/19 06:50:25 marquet Exp $
   ------------------------------------------------------------
...
void
jobs_clearjob(struct job_t *job)
{
    job->jb_pid = 0;
    job->jb_jid = 0;
    job->jb_state = UNDEF;
	    ...
	    strcpy(jobs[i].jb_cmdline, cmdline);
  	    if(verbose){
	        printf("Added job [%d] %d %s\n",
		       jobs[i].jb_jid, (int) jobs[i].jb_pid, jobs[i].jb_cmdline);
            }
            return 1;
	}
    }
